...
import 'package:we_lean/bloc/daywise_ppc_list/daywise_ppc_list_state.dart';
...
class PPCSummaryWeek extends StatefulWidget {
  
  final int week_plan_id;
  
  const PPCSummaryWeek({required this.week_plan_id,Key? key}) : super(key: key);

  @override
  _PPCSummaryWeekState createState() => _PPCSummaryWeekState();
}

class _PPCSummaryWeekState extends State<PPCSummaryWeek> {


  List<PPCSummary>  _ppcSummaryList=[
    PPCSummary("Mon", "11-03-24", 5, 4, 90),
    PPCSummary("Tue", "12-03-24", 3, 3, 100),
    PPCSummary("Wed", "13-03-24", 2, 2, 100),
    PPCSummary("Thurs", "14-03-24", 6, 5, 90),
    PPCSummary("Fri", "15-03-24", 3, 2, 90),
    PPCSummary("Sat", "16-03-24", 1, 1, 100),
    PPCSummary("Sun", "17-03-24", 5, 4, 90),
  ];
  ...
  void initState() {
    ...
    BlocProvider.of<DayWisePPCBloc>(context).add(FetchDayWisePPC(weekPlanId: widget.week_plan_id));
    super.initState();
  }
...
class PPCSummary{
  ...
  PPCSummary(this.day,this.date,this.plannedActivities,this.achievedActivities,this.ppc);
